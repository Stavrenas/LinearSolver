...
int mm_read_vec_crd_size(FILE *f, int *M)
{
    char line[MM_MAX_LINE_LENGTH];
    int num_items_read;
    ...
    *M = 0;
    ...
    /* line[] is either blank or has M,secondDimension */
    if (sscanf(line, "%d %d", M, &secondDimension) == 2)
        ...
        do
        {
            num_items_read = fscanf(f, "%d %d", M, &secondDimension);
            if (num_items_read == EOF)
                return MM_PREMATURE_EOF;
